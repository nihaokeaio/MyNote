...
void MyShader::loadCubeTexture(const std::vector<std::string>& cubeFileName, unsigned int& texture)
{
    //create texture object;
    glGenTextures(1, &texture);
    ...
    for (int i = 0; i < cubeFileName.size(); ++i)
    {     
        unsigned char* data = stbi_load(cubeFileName[i].c_str(), &width, &height, &nChannal, 0);
        if (data)
        {
            GLenum format = GL_RGB;
            if (nChannal == 1)
...
void MyShader::setVec2(const std::string& name, glm::vec2 vec2) const
{

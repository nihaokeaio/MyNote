...
### 判断鼠标是否在组件内
问题：一个矩形组件内部有一个子的Ivierew组件框，需要判断鼠标离开这整个组件的事件，以此设置样式
```c++
auto mp = selectTemplateWin_->mapFromGlobal(QCursor::pos());
if (selectTemplateWin_->rect().contains(mp))
{				
	return true;
}
return false;
```
这里首先将鼠标位置映射到这整个组件的相对位置，然后判断该位置是否在组件内部，配合上鼠标离开事件```QEvent::Leave```,就可以完成判断
...
核心就是如何将二维的屏幕坐标转换为三维坐标实现场景中模型的旋转，平移
...
1. 旋转变换
	...
	Vec3f TransMeshHandler::PImpl::projectToTrackball(const Point2f& screenCoords) const
	{
		...
		//将交点与mv矩阵相乘，得到相机视角下的空间位置。
		return Matrix44f::transform3x3(render_->getModelViewMatrix(), Vec3f(p2d.X(), p2d.Y(), z));
	}
	```
传入二维的屏幕坐标，首先将其归一化为$[-0.5,0.5]^2$的屏幕坐标，之后，根据轨迹球的半径，判断其是否在轨迹球内，
对于轨迹球内的滑动点，我们通过球的隐式方程求解出z值，最终反映射出对应的（x,y,z）值。若在球外，我们使用双曲线
函数求解z值。其数学方程为：![轨迹球求解公式](./resource/TrackballMath.png)

注意，这里求解的是鼠标滑动一定距离后轨迹球的假想点。它与初始的假想点以及原点共同构建了两个向量。因此，问题就转换成了
我们有两个向量***v1,v2***,计算得到的旋转矩阵。这在一般的矩阵变换中都有相应的接口。因此我们可以得到对应的旋转矩阵。
```C++
...
if (hitMesh_)
{
	rootNode_->setMatrix(nodeTMat_ * mR * nodeRMat_);
}
else
{
	rootNode_->setMatrix(mR * nodeMat_);
}
```
这里需要注意的是：对于鼠标命中模型的情况，我们是对模型原有的旋转矩阵进行旋转后，在移动到对应位置的。这表现为模型井仔自身坐标系下进行旋转。
当鼠标未命中模型时，我们直接对模型进行旋转矩阵的变换，这表现为对坐标原点的旋转。当然，这里也可以是对鼠标位置进行旋转，但这都是可选项。
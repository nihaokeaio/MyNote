...
OcTree::OcTree(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud)
{
	...
	octree_.reset(new pcl::octree::OctreePointCloudSearch<pcl::PointXYZ>(resolution));
	octree_->setInputCloud(cloud);
	octree_->addPointsFromInputCloud();
}

bool OcTree::queryVoxel(const pcl::PointXYZ& point, std::vector<int>& indexList)
{
	if (octree_->voxelSearch(point, indexList))
	{
		return true;
	}
	return false;
...
bool OcTree::queryKNearest(const pcl::PointXYZ& point, int k, std::vector<int>& indexList, std::vector<float>& distList)
{
	indexList.resize(k);
	distList.resize(k);
	if (octree_->nearestKSearch(point, k, indexList, distList) > 0)
	{
		return true;
	}
	return false;
...
bool OcTree::queryRadius(const pcl::PointXYZ& point, float radius, std::vector<int>& indexList, std::vector<float>& distList)
{
	if (octree_->radiusSearch(point, radius, indexList, distList) > 0)
	{
		return true;
	}
	return false;

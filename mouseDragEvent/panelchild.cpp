...
Panelchild::Panelchild(QWidget *parent) : QWidget(parent)
{
    ...
    //connect(addButton,SIGNAL(clicked()),this,SLOT(slotAddItem()));
    //connect(subButton,SIGNAL(clicked()),this,SLOT(slotSubItem()));
...
//spanAngle 弧的跨度
//arcHeight 弧的宽度
SubGphItem *Panelchild::gradientArc(int radius, qreal startAngle, qreal spanAngle, int arcHeight)
{
...
void Panelchild::buildArcPanel()
{
    ...
    sceneRect->setRect(QRect(-CIRCLE_RADIUS_-RECT_GAP_SPACING_,-CIRCLE_RADIUS_-RECT_GAP_SPACING_,(CIRCLE_RADIUS_+RECT_GAP_SPACING_)*2,(CIRCLE_RADIUS_+RECT_GAP_SPACING_)*2));
    ...
    for (int i=0;i<distributId;i++) {
        ...
        gphitem=gradientArc(CIRCLE_RADIUS_,360.f/aliveObj*numbblock,360.f/aliveObj-CIRCLE_GAP_SPACING_,CIRCLE_WIDTH_);
        ++numbblock;
...
//    auto iter=m_obj_list.find(deleteObj);
//    if(iter!=m_obj_list.end())
    {
